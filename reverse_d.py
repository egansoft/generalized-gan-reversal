...
def reverse_z(netG, netD, x, z, opt, lam=0, clip='disabled'):
  """
...
#    print netD(x_approx)
    mse_x = mse_loss(x_approx, x) + (1- netD(x_approx))
#    print mse_x
    if i % 1000 == 0:
...
def reverse_gan(opt):
  # load netG and netD
  ngpu = int(opt.ngpu)
  netG = gan.Generator(ngpu, opt.nc, opt.nz, opt.ngf, opt.netG)
  netD = gan.Discriminator(ngpu, opt.nc, opt.ndf, opt.netD)
  ...
  for i in xrange(epochs):
    ...
    for li, lam in enumerate(lams):
      z_approx, losses = reverse_z(netG, netD, x, z, opt, lam=lam, clip=opt.clip)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--netD', default='sampleDparams.pth',
